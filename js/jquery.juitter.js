					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<a href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /> <span class='tweeter'>"+item.from_user+"</span></a> "+$.Juitter.textFormat(tweet)+"  <span class='time'>"+item.created_at+"</span><hr class='twitterRule' />";// |- <a href='" + link + "' class='JRM' "+openLink+">"+readMore+"</a>";
								else mHTML="<a href='http://www.twitter.com/"+item.from_user+"'>@"+item.from_user+":</a> "+$.Juitter.textFormat(tweet)+" -| <span class='time'>"+item.created_at+"</span><hr class='twitterRule' />";// |-  <a href='" + link + "' "+openLink+">"+readMore+"</a>";
