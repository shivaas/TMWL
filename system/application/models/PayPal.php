...
/*
	**************************************************
	Angell EYE © 2009 : Class v1.0
	PayPal Pro NVP API Documentation : June 2009
	API Version 58.0
	**************************************************
...
class PayPal
{
	...
	function PayPal($DataArray)
	{
		...
		$this -> APIVersion = isset($DataArray['APIVersion']) ? $DataArray['APIVersion'] : '62.0';
		...
		if($this -> Sandbox || $this -> BetaSandbox)
		{
			...
			{
				# Beta Sandbox
				$this -> APIUsername = isset($DataArray['APIUsername']) && $DataArray['APIUsername'] != '' ? $DataArray['APIUsername'] : 'usbswi_1213841674_biz_api1.angelleye.com';
				$this -> APIPassword = isset($DataArray['APIPassword']) && $DataArray['APIPassword'] != '' ? $DataArray['APIPassword'] : '1213841681';
				$this -> APISignature = isset($DataArray['APISignature']) && $DataArray['APISignature'] != '' ? $DataArray['APISignature'] : 'A3XwtUgn.4zxIQFE4uCKENrIsjjAA-PCtP9cU-vQoARmq-1EOqkQ6Do.';
			...
			{
				#Sandbox
				$this -> APIUsername = isset($DataArray['APIUsername']) && $DataArray['APIUsername'] != '' ? $DataArray['APIUsername'] : 'sandbo_1215254764_biz_api1.angelleye.com';
				$this -> APIPassword = isset($DataArray['APIPassword']) && $DataArray['APIPassword'] != '' ? $DataArray['APIPassword'] : '1215254774';
				$this -> APISignature = isset($DataArray['APISignature']) && $DataArray['APISignature'] != '' ? $DataArray['APISignature'] : 'AiKZhEEPLJjSIccz.2M.tbyW5YFwAb6E3l6my.pY9br1z2qxKx96W18v';
		...
		{
			$this -> APIUsername = isset($DataArray['APIUsername']) && $DataArray['APIUsername'] != '' ? $DataArray['APIUsername'] : 'admin_api1.epicchange.org';
			$this -> APIPassword = isset($DataArray['APIPassword']) && $DataArray['APIPassword'] != ''  ? $DataArray['APIPassword'] : '36CD5H3YVTSD5LC7';
			$this -> APISignature = isset($DataArray['APISignature']) && $DataArray['APISignature'] != ''  ? $DataArray['APISignature'] : 'AFcWxV21C7fd0v3bYYYRCpSSRl31AXbFH8fzy4MlZbETuezxFSoUBgZW';
	...
	*/
	...
	function SetExpressCheckout($DataArray)
	{
		...
		array_push($RecurringPayments, $Item);
		...
		*/		
		...
		foreach($BillingAgreements as $BillingAgreementVar => $BillingAgreementVal)
		{
			...
			foreach($CurrentItem as $CurrentItemVar => $CurrentItemVal)
				$BillingAgreementsNVP .= '&' . strtoupper($CurrentItemVar) . $n . '=' . $CurrentItemVal;
			$n++;	
		}
		...
		foreach($ShippingOptions as $ShippingOptionsVar => $ShippingOptionsVal)
		{
			...
			foreach($CurrentOption as $CurrentOptionVar => $CurrentOptionVal)
				$ShippingOptionsNVP .= '&' . strtoupper($CurrentOptionVar) . $n . '=' . $CurrentOptionVal;
			$n++;	
		}
		
		$NVPRequest = $this -> NVPCredentials . $SECFieldsNVP . $PaymentDetailsNVP . $OrderItemsNVP . $RecurringPaymentsNVP . $ShippingAddressNVP . $ShippingOptionsNVP . $BillingAgreementsNVP;
		$NVPResponse = $this -> CURLRequest($NVPRequest);
		$NVPRequestArray = $this -> NVPToArray($NVPRequest);
		$NVPResponseArray = $this -> NVPToArray($NVPResponse);
	...
	function CallbackResponse($DataArray)
	{
		...
		foreach($CBFields as $CBFieldsVar => $CBFieldsVal)
			$CBFieldsNVP .= '&' . strtoupper($CBFieldsVar) . '=' . $CBFieldsVal;
	...
	function DoExpressCheckoutPayment($DataArray)
	{
		...
		$Item		 = array(
							'l_name' => '', 							// Item name. 127 char max.
							'l_desc' => '', 								// Item description.  127 single char max.
							'l_amt' => '', 								// Cost of item.
							'l_number' => '', 							// Item number.  127 char max.
							'l_qty' => '', 								// Item qty on order.  Any positive integer.
							'l_taxamt' => '', 							// Item sales tax
	...
	*/
	...
	function DoReferenceTransaction($DataArray)
	{
		...
		foreach($DRTFields as $DRTFieldsVar => $DRTFieldsVal)
			$DRTFieldsNVP .= '&' . strtoupper($DRTFieldsVar) . '=' . $DRTFieldsVal;
	...
	*/
	...
	function GetPalDetails($DataArray)
	{
		...
		foreach($GPFields as $GPFieldsVar => $GPFieldsVal)
			$GPFieldsNVP .= '&' . strtoupper($GPFieldsVar) . '=' . $GPFieldsVal;
	...
	function CreateRecurringPaymentsProfile($DataArray)
	{
		...
		*/
		...
		foreach($CRPPFields as $CRPPFieldsVar => $CRPPFieldsVal)
			$CRPPFieldsNVP .= '&' . strtoupper($CRPPFieldsVar) . '=' . $CRPPFieldsVal;
	...
	*/
	...
	function SetAuthFlowParam($DataArray)
	{
		...
		foreach($SetAuthFlowParamFields as $SetAuthFlowParamFieldsVar => $SetAuthFlowParamFieldsVal)
			$SetAuthFlowParamFieldsNVP .= '&' . strtoupper($SetAuthFlowParamFieldsVar) . '=' . $SetAuthFlowParamFieldsVal;
		...
		foreach($ShippingAddressFields as $ShippingAddressFieldsVar => $ShippingAddressFieldsVal)
			$SetAuthFlowParamFieldsNVP .= '&' . strtoupper($ShippingAddressFieldsVar) . '=' . $ShippingAddressFieldsVal;
	...
	function SetAccessPermissions($DataArray)
	{
		...
		foreach($SetAccessPermissionsFields as $SetAccessPermissionsFieldsVar => $SetAccessPermissionsFieldsVal)
			$SetAccessPermissionsNVP .= '&' . strtoupper($SetAccessPermissionsFieldsVar) . '=' . $SetAccessPermissionsFieldsVal;
		
		$n = 0;
		$RequiredPermissions = isset($DataArray['RequiredPermissions']) ? $DataArray['RequiredPermissions'] : array();
		foreach($RequiredPermissions as $RequiredPermission)
		{
			$SetAccessPermissionsNVP .= '&L_REQUIREDACCESSPERMISSIONS' . $n . '=' . $RequiredPermission;
			$n++;
		}
		...
		{
			$SetAccessPermissionsNVP .= '&L_OPTIONALACCESSPERMISSIONS' . $n . '=' . $OptionalPermission;
			$n++;
		}
